...
export function normalizeListing(listing: any): NormalizedProperty {
  // Sanitize title — remove 'null' artifacts from constructed titles
  let title = listing.title || listing.address || '';
  if (!title || title.toLowerCase().startsWith('null')) {
    const type = listing.propertyType || 'Property';
    const city = listing.city || '';
    title = city ? `${type} in ${city}` : type;
  }
  ...
  return {
    ...
    propertySubType: listing.propertySubType || listing.propertyType || '',
...
export function normalizeMLSProperty(listing: any): NormalizedProperty {
  const imageUrl =
    listing.Media?.[0]?.MediaURL || listing.primaryPhotoUrl || listing.primaryPhoto || '';

  // Construct a clean title — handle null/empty address
  let title = listing.UnparsedAddress || '';
  if (!title || title.toLowerCase().startsWith('null')) {
    const type = listing.PropertySubType || listing.PropertyType || 'Property';
    const city = listing.City || '';
    title = city ? `${type} in ${city}` : type;
  }
