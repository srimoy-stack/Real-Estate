...
export const Header: React.FC = () => (
    <header className="sticky top-0 z-50 bg-white">
        <div className="max-w-5xl mx-auto px-6 h-20 flex items-center justify-between">
            <Link href="/" className="text-lg font-light text-slate-900 tracking-wide">minimal<span className="font-bold">realty</span></Link>
            <nav className="hidden md:flex items-center gap-8">
                {['Properties', 'About', 'Contact'].map(l => (
                    <Link key={l} href={l === 'Properties' ? '/listings' : '#'} className="text-sm text-slate-400 hover:text-slate-900 transition-colors">{l}</Link>
                ))}
            </nav>
        </div>
