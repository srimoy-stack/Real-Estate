...
export async function GET(
    ...
    try {
        ...
        const mapShortcodeToSearchFilters = () => {
            ...
            setIf('minPrice', searchParams.get('minPrice') ? parseFloat(searchParams.get('minPrice')!) : f.minPrice);
            setIf('maxPrice', searchParams.get('maxPrice') ? parseFloat(searchParams.get('maxPrice')!) : f.maxPrice);
            setIf('beds', searchParams.get('bedrooms') ? parseInt(searchParams.get('bedrooms')!, 10) : f.bedrooms);
            setIf('baths', searchParams.get('bathrooms') ? parseInt(searchParams.get('bathrooms')!, 10) : f.bathrooms);
            setIf('featured', searchParams.get('featured') ? searchParams.get('featured') === 'true' : f.featured);
        ...
        const { listings: listingsRaw } = await getListingsForShortcode(
            prisma,
            baseQuery,
            limit
        );
        ...
        const mappedListings = listingsRaw.map((listing: any) => {
            ...
            return {
                ...
                Media: listing.mediaJson || raw.Media || (listing.primaryPhotoUrl ? [{ MediaURL: listing.primaryPhotoUrl, PreferredPhotoYN: true }] : []),
        ...
        return NextResponse.json({ error: 'Failed to resolve shortcode', message: err.message }, { status: 500 });
    }
}
