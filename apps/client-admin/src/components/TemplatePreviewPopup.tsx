...
import { PLATFORM_TEMPLATES } from '@repo/services';
...
export function TemplatePreviewPopup({ templateKey, onClose }: TemplatePreviewPopupProps) {
    const template = PLATFORM_TEMPLATES.find(t => t.templateKey === templateKey);
    ...
    return (
        <div className="fixed inset-0 z-[500] bg-slate-950/40 backdrop-blur-md flex flex-col overflow-hidden animate-in fade-in duration-300 p-4 md:p-10">
            <div className="bg-white w-full h-full rounded-[40px] shadow-2xl flex flex-col overflow-hidden border border-slate-200">
                <header className="h-16 bg-slate-900 flex items-center justify-between px-6 shrink-0">
                    <div className="flex items-center gap-4">
                        ...
                        <div>
                            <h1 className="text-white font-black text-xs">Live <span className="text-indigo-400">Design Preview</span></h1>
                ...
                <main className="flex-1 relative flex justify-center bg-slate-50 overflow-hidden pt-10">
                    ...
                    >
                        {/* Use iframe to render the real template via the public demo route */}
                        <iframe
