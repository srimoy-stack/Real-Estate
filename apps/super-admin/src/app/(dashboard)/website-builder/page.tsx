...
const NavigationPanel = ({ websiteId, onUpdate }: { websiteId: string; onUpdate: () => void }) => {
    const [nav, setNav] = React.useState<{ label: string; slug: string }[]>([]);
    const [loading, setLoading] = React.useState(true);

    const fetchData = async () => {
        const website = await orgWebsiteService.getOrgWebsite(websiteId);
        if (website?.navigation) setNav(website.navigation);
        setLoading(false);
    };
    ...
    const handleSave = async () => {
        await orgWebsiteService.updateNavigation('org-1', websiteId, nav);
        onUpdate();
    };

    const addLink = () => setNav([...nav, { label: 'New Link', slug: '/' }]);
    const removeLink = (idx: number) => setNav(nav.filter((_, i) => i !== idx));
        ...
        <div className="p-5 space-y-6">
            <div className="flex items-center justify-between">
                <div>
                    ...
                    <p className="text-[10px] text-slate-500 font-medium">Configure header menu</p>
                </div>
                <button onClick={addLink} className="p-2 bg-indigo-50 text-indigo-600 rounded-lg hover:bg-indigo-600 hover:text-white transition-all">
                    <svg className="w-4 h-4" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M12 4v16m8-8H4" /></svg>
                </button>
            </div>
...
const SettingsPanel = () => {
    ...
    const handleDuplicateSelected = () => {
        ...
        try {
            ...
            const comp = typeof componentType === 'string' ? (Sections as any)[componentType] : componentType;
