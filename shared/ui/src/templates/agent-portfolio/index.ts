// Layout & Navigation
export { Header, Footer, TemplateLayout } from './layout';

// Pages
export { Homepage } from './homepage';
export { ListingsPage } from './listings-page';
export { ListingDetailPage } from './listing-detail-page';

// Individual Components (for granular use)
export { AgentHero, AgentHero as Hero } from './agent-hero';
export { AgentBio } from './agent-bio';
export { ListingCard } from './listing-card';
export { FeaturedListings as Listings } from './FeaturedListings';
export { CommunitiesSection } from './communities-section';
export { Testimonials } from './testimonials';
export { ContactSection as Contact } from './contact-section';
