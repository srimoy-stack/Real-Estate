...
export const LuxuryHero: React.FC<any> = ({
    ...
    const title = content?.headline || headline;
    const subtitle = content?.subheadline || subheadline;
                ...
                <div className="max-w-4xl">
                    ...
                    <h1 className="text-6xl md:text-8xl lg:text-9xl font-black text-white tracking-tighter leading-[0.85]">
                        {title.split(' ').map((word: string, i: number) => (
                            <span key={i} className={`block ${i === 1 ? 'text-amber-400 italic' : ''}`}>{word}</span>
                        ))}
                    </h1>
